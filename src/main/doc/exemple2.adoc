...
* l'émission d'une erreur (de type 404 - NOT_FOUND) si l'identifiant ne correpond à un aucun étudiant, puisqu'il est techniquement possible - contrairement à l'exemple 1 - que la vue tente d'accéder à un élément du modèle qui n'existe pas (essayer avec http://localhost:8080/mvcexample/etudiant/100)
...
Puisqu'elle est associée à une logique concernant un élément du *modèle*, nous devons l'intégrer à cette partie.

[NOTE]
====
De la même manière que les contrôleurs, cette classe doit être gérée par *Spring*. Pour cela :
